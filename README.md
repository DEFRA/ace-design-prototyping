...
> [!WARNING]
> The `ace-design-prototyping` is not a production ready application, it is a tool for prototyping. It is not
...
The refreshed GOV.UK brand is available and turned on by default in the `ace-design-prototyping`. To turn it
...
```bash
docker build --target development --no-cache --tag ace-design-prototyping:development .
```

Run:

```bash
docker run -e PORT=3000 -p 3000:3000 ace-design-prototyping:development
```
...
```bash
docker build --no-cache --tag ace-design-prototyping .
```
...
```bash
docker run -e PASSWORD=beepBoopBeep -e PORT=3000 -p 3000:3000 ace-design-prototyping
```
...
```bash
docker run -it --entrypoint /bin/ash ace-design-prototyping
```
